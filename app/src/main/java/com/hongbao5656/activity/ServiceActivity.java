...
public class ServiceActivity extends BaseActivity implements HttpDataHandlerListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.mContext = ServiceActivity.this;
...
//        initListener();
    ...
    public void initDatas() {
        sendGetRequest(
                Constants.download,
                new HttpRequest(new Request.Builder().url("http://www.56hb.net/api/Version/hytx").build()),
                this);
    }
                        ...
                        switch (which) {
                            case DialogInterface.BUTTON_POSITIVE:
                                ...
                                if (SU.isEmpty(et_login_tel.getText().toString())) {
                                    TU.show(ServiceActivity.this, "邀请码不能为空！");
                                    return;
                                }
...
//                    TU.show(this,"");
    ...
    public void setHandlerGetDataSuccess(int connectionId, Object data) throws JSONException {
        if (Constants.download == connectionId) {//{"version":"1.2.1","content":"1.界面优化\r\n2.bug修复"}
            ...
            int versioncode = Integer.parseInt((JSON.parseObject(data.toString()).get("version")).toString());
            String content = JSON.parseObject(data.toString()).get("content").toString();
            if (SU.getVersionCodeAndVersionName(mContext).versionCode >= versioncode) {
    ...
    private void showShare() {
        ShareSDK.initSDK(mContext);//ShareSDK初始化
