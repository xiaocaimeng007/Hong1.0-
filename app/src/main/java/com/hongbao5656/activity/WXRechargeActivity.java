...
public class WXRechargeActivity extends BaseActivity implements
        HttpDataHandlerListener,IWXAPIEventHandler {
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_wxpayrecgarge);
        setViews();
    }
...
//		payBtn.setEnabled(false);
        Toast.makeText(WXRechargeActivity.this, "获取订单中...", Toast.LENGTH_SHORT).show();
        try{
...
//            requestServer();
            ...
            Toast.makeText(WXRechargeActivity.this, "异常："+e.getMessage(), Toast.LENGTH_SHORT).show();
        }
            ...
            switch (msg.what) {
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        Toast.makeText(WXRechargeActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
//                        finish();
                    } else {
                        // 判断resultStatus 为非"9000"则代表可能支付失败
                        // "8000"代表支付结果因为支付渠道原因或者系统原因还在等待支付结果确认，最终交易是否成功以服务端异步通知为准（小概率状态）
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(WXRechargeActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(WXRechargeActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                }
                        ...
                        public void onResponse(Response response) throws IOException {
                            if (response.code() == 200) {
                                ...
                                Runnable payRunnable = new Runnable() {
                                    ...
                                    public void run() {
                                        // 构造PayTask 对象
                                        PayTask alipay = new PayTask(WXRechargeActivity.this);
                                        // 调用支付接口，获取支付结果
